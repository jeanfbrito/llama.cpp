...
//
// Algorithm overview:
//   1. Load GROUP_SIZE floats coalesced (one thread per element)
//   2. Parallel L2 norm (warp reduce + inter-warp via shared mem)
//   3. Normalize
//   4. Forward WHT (signs1 → butterfly → signs2, normalized)
//   5. Quantize to 3-bit centroid (ternary: {-1,0,+1} × scale, 8 centroids)
