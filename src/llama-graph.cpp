...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // V padding removal: only applies when K is also turbo (FA path, non-transposed V).
    // When K is upgraded to q8_0 via auto-asymmetric, non-FA is used and V is transposed
    // (v->ne[0] = n_kv, not n_embd_head_v), which would break the reshape below.
    if ((v->type == GGML_TYPE_TURBO3_0 || v->type == GGML_TYPE_TURBO4_0 || v->type == GGML_TYPE_TURBO2_0) &&
